...
public class ReptySample {
  ...
  /**
   * ページは、自分で設定する。 PDPageを生成するメソッド Map<String, Object> param, cs, resourcesocument, Templateを渡す
   * drawだけじゃじゃなくて、共通と各ページのキーとなる情報でまとめる。 可変表と線の対応 変数のみの設定を実施する。 文字レコード指定で、 レコードを指定して、変数を取得
   * type:record values: 1,2,0,10,a,リスト1
   * offsetX,offsetY,nextX,nextY,dtoのメンバ変数名、list名（parameterMapで指定) 文字列出力
   * offsetX1,offsetY1,offsetX2,offsetY2,nextX,nextY,list名 線出力繰り返しタイプ複数線、一直線nextの仕方で判別可能
   * offsetX1,offsetY1,offsetX2,offsetY2,nextX,nextY,list名 rectangle出力、サイズを可変にする。繰り返しのパターンはない。
   * いずれもmax値の制限がほしい。見た目を変えるのはtoStringで実装 右寄せ、左寄せ、中央寄せは、共通で実装 この処理は、commonとpageAを実施という風に変更
   *
    ...
    for (int i = 1; i >= 0; i--) {
      try (PDDocument document = new PDDocument(MemoryUsageSetting.setupMainMemoryOnly()); ) {
        ...
        try (Repty yamlPdf = new Repty(document, template); ) {
          ...
          System.out.println(
              (System.currentTimeMillis() - start) + "[msec]yamlPdf create page 1 file");
          start = System.currentTimeMillis();
          ...
          try (BufferedOutputStream fos =
              new BufferedOutputStream(
                  new FileOutputStream(outFile))) { // bufferingすると300msec右40msecになる
            document.save(fos);
          }
          ...
          start = System.currentTimeMillis();
          System.out.println(
              (System.currentTimeMillis() - forstart) + "[msec]yamlPdf process 1 file");
