			...
			case OBJECT:
				...
				for (Value value : draw.getValues()) {
					...
					if (value.getLimitX() > 0) {
						...
						do {
							nextLineIndex = DrawUtil.getNextLineIndex(pdFont, font2.getSize(), tempValue.substring(currentIndex),
									limitWidth);
						...
						// 横寄せ
						float x = DrawUtil.getAlignOffset(value.getX1(),
								DrawUtil.getPdfboxSize(font2.getSize(), pdFont.getStringWidth(tempValue)), value.getAlignX());
						// 縦寄せ
						float y = DrawUtil.getAlignOffset(value.getY1(),
								DrawUtil.getPdfboxSize(font2.getSize(), pdFont.getFontDescriptor().getCapHeight()),
								value.getAlignY());
						stream.newLineAtOffset(x, y);
