	...
	public void draw(PDPageContentStream stream, Map<String, Object> paramMap) throws Exception {
		...
		for (Value value : draw.getValues()) {
			...
			if (value.getLimitX() > 0) {
				...
				do {
					nextLineIndex = DrawUtil.getNextLineIndex(pdFont, fontSize, tempValue.substring(currentIndex),
							limitWidth);
				...
				// 横寄せ
				float x = DrawUtil.getAlignOffset(value.getX1(),
						DrawUtil.getPdfboxSize(fontSize, pdFont.getStringWidth(tempValue)), value.getAlignX());
				// 縦寄せ
